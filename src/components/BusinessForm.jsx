    ...
    return (
        <Dialog open={open} onClose={!isSubmitting ? onClose : () => {}} fullWidth maxWidth="md">
            ...
            <DialogContent>
                ...
                {!apiError && packages && (
                    <RadioGroup value={selectedPackageId} onChange={(e) => setSelectedPackageId(e.target.value)} sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2, mt: 2 }}>
                        {packages.map((pkg) => (
                            <Card key={pkg.id} variant="outlined" onClick={() => setSelectedPackageId(pkg.id)} sx={{ flex: 1, cursor: 'pointer', borderRadius: '12px', border: selectedPackageId === pkg.id ? '2px solid' : '1px solid', borderColor: selectedPackageId === pkg.id ? 'primary.main' : 'grey.300', boxShadow: selectedPackageId === pkg.id ? '0 4px 12px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s ease-in-out', '&:hover': { borderColor: 'primary.light', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' } }}>
                                <CardContent sx={{ p: 3, textAlign: 'center' }}>
                                    <FormControlLabel value={pkg.id} control={<Radio sx={{ mb: 1 }} />} label={<Typography variant="h5" component="div" fontWeight="bold">{pkg.name}</Typography>} sx={{ justifyContent: 'center', width: '100%', ml: 0 }} />
                                    <Typography variant="h4" color="primary.main" sx={{ my: 1, fontWeight: '700' }}>{pkg.totalPrice}</Typography>
                                    <Typography variant="body2" color="text.secondary">{`Package: ${pkg.price} + State Fee: ${pkg.stateFees}`}</Typography>
                                    <Box sx={{ my: 2, textAlign: 'left', px: 1 }}>
                                        {/* Use the transformed featureGroups structure */}
                                        {pkg.featureGroups.find(fg => fg.groupName === 'What You Get!')?.groupFeatures.map(feature => (
            ...
            <DialogActions sx={{ p: 3, justifyContent: 'center', gap: 2 }}>
                <Button onClick={onClose} color="secondary" disabled={isSubmitting} sx={{ textTransform: 'none', fontSize: '1rem' }}>Cancel</Button>
...
const BusinessForm = () => {
    ...
    const handleFormSubmit = async (values) => {
        ...
        try {
            ...
            if (data.statusCode === "200" && data?.value?.packageCollection) {
                ...
                // Flatten and transform all productPackages inside packageCollection
                const transformedPackages = rawPackages.flatMap(pkgCol => {
                  return pkgCol.productPackages.map(pkg => {
                    ...
                    // Handle productOptions safely
                    const productOptions = pkg.productOptions || [];
              
                    // Required state and 3rd party fees
                    ...
                    return {
                      ...
                      displayOrder: pkg.collectionDisplaySequence || 0 // ensure fallback
                    };
                  });
                }).sort((a, b) => a.displayOrder - b.displayOrder);
              
                setPackagesData(transformedPackages);
              } else {
    ...
    return (
        <>
            <PackageModal open={isModalOpen} onClose={() => setModalOpen(false)} packages={packagesData} onContinue={handleContinueToPayment} apiError={packageApiError} />
